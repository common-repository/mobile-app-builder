...
add_shortcode('mab_submission_form', 'mab_add_new_post');

/**
 * Creates shortcode mab_article_list
 *
 * @return string: HTML content for the shortcode
 */
function mab_manage_posts()
{
	$mab_misc = get_option('mab_misc');
	if (!is_user_logged_in()) {
		include(dirname(dirname(__FILE__)) . '/views/login.php');
...
add_shortcode('mab_article_list', 'mab_manage_posts');



function mab_app_render_paypal_button_with_other_amt($args) {
	...
	$email = apply_filters('mab_app_widget_any_amt_email', $email);
	...
	$output .= '<div class="wp_pp_button_amount_section">'.esc_attr($other_amount_label).' <input type="number" min="1" step="any" name="amount" value="' . esc_attr($default_amount) . '" class="mab_app_other_amt_input" style="max-width:80px;"> ' . esc_attr($currency) . '</div>';
	...
	if (!empty($validate_ipn)) {
		$notify_url = site_url() . '/?mab_app_paypal_ipn=process';
	...
	if (!empty($button_text)) {//Use text button
		$button_class = apply_filters('mab_app_text_button_class', '');
...
function mab_app_render_paypal_button_form($args) {
	extract(shortcode_atts(array(
				'email' => '',
				'currency' => 'USD',
	...
	$email = apply_filters('mab_app_widget_email', $email);
	if (empty($email)) {
		$output = '<p style="color: red;">Error! Please enter your PayPal email address for the payment using the "email" parameter in the shortcode</p>';
		return $output;
	}
...
?>
	...
	if (!empty($other_amount)) {
		...
		echo esc_attr($other_amount_label).' <input type="number" min="1" step="any" name="other_amount" value="" class="mab_app_other_amt_input" style="max-width:80px;"> ' . esc_attr($currency);
		echo '</div>';
	}
